...
})->name('landing.subscription.expired');
...
})->name('landing.faq.index');

Route::get('/event/{id}/{slug?}', function (string $id) {
    $client = new GraphQLClient();
    $query = <<<'GRAPHQL'
...
})->name('landing.news.detail');

Route::get('/galeri/{id}', function (string $id) {
    $client = new GraphQLClient();
    $query = <<<'GRAPHQL'
